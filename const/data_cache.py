import datetime

from domain.yellow_dust.yellow_dust import YellowDust
from utils.time_utils import string_to_datetime_air_quality, string_to_date_include_year

last_covid_date: datetime.date = datetime.date.min
last_covid_update_date: datetime.date = datetime.date.min
last_air_quality_datetime = ""
last_yellow_dust = YellowDust()
last_cold_value = 0
last_cold_date = datetime.date.min


def get_datetime_last_air_quality_datetime():
    if last_air_quality_datetime == "":
        return datetime.datetime.min
    else:
        return string_to_datetime_air_quality(last_air_quality_datetime)


def get_last_covid_update_date() -> datetime.date:
    return last_covid_update_date

def get_last_covid_date() -> datetime.date:
    return last_cold_date

def get_date_last_yellow_dust() -> datetime.datetime:
    return last_yellow_dust.datetime
